...
직접 쓰는 묵상집은 **관리자 화면에서 만드는 것이 가장 빠릅니다.** `/admin` → **묵상집 관리** 탭 → **+ 새 묵상집** 에서 이름(한국어·영어)과 소개, 탭 순서를 넣고 저장하면 `collections.json` 에 바로 커밋됩니다. 만든 뒤 **여기에 글 쓰기** 를 누르면 그 묵상집이 선택된 채로 글쓰기 화면이 열리고, 그다음부터는 날짜만 바꿔 가며 매일 한 편씩 쓰면 됩니다.

이메일·웹에서 가져오는 묵상집은 `importer` 설정이 필요하므로 아래처럼 `collections.json` 에 직접 적습니다. (관리자 화면에서도 이름과 소개는 고칠 수 있지만, `importer` 설정은 건드리지 않습니다.)
...
글이 한 편도 없는 묵상집은 독자 화면의 탭에 나타나지 않습니다. 관리자 화면에는 늘 보이므로, 만들어 두고 첫 글을 쓰면 그때 탭이 생깁니다. 지우는 것도 글이 하나도 없을 때만 됩니다 — 글이 남아 있으면 먼저 글을 지우라고 알려 줍니다.
...
`.github/workflows/import-daily.yml` 이 `America/New_York` 기준 매일 **오전 7:05**에 Tozer 수집을 예약합니다. 서머타임 전환도 자동 반영합니다. 메일 도착 지연이나 일시적 장애에 대비해 **오전 7:35와 8:05**에도 누락분을 확인합니다. 최근 3일 안에서 아직 없는 것만 최대 2편을 가져오며, 이미 저장된 날짜에는 Gemini를 호출하지 않습니다. Actions 탭에서 손으로 돌릴 수도 있습니다.

GitHub Actions의 예약 실행은 정각 실행을 보장하지 않으며 수 시간 지연되거나 누락될 수 있습니다. 보조 일정도 같은 제한이 있습니다. 정확한 시각의 실행이 필수라면 외부 스케줄러에서 `workflow_dispatch`를 호출하도록 별도 운영 설정이 필요합니다. 코드 변경은 GitHub 기본 브랜치에 반영된 뒤부터 적용됩니다.

Gemini의 일시적 서버 오류(500/502/503/504)와 429는 20초, 40초 간격으로 최대 두 번 재시도합니다. 재시도도 실행당 호출 예산에 포함합니다. 수집 오류, 호출 한도 소진, 또는 오늘의 한국어·영어 묵상이 없는 경우 실행을 실패로 표시합니다. 실패 전 저장된 글은 커밋하여 보존하고, 다음 예약 실행에서 미완료 날짜를 다시 처리합니다.
...
호출 예산은 기본적으로 **실행당 18회**입니다. 재시도가 없으면 글 한 편에 한 번 호출합니다. 실제 API 할당량은 사용 중인 모델과 계정 등급에 따라 다르며, 실행당 예산은 하루 전체 사용량을 합산하지 않습니다. 밀린 것이 많으면 다음 날 다시 실행하면 됩니다 — 이미 있는 날짜는 건너뜁니다. 한도에 걸려 멈추면 워크플로는 실패로 표시되지만 그때까지 저장한 것은 커밋됩니다.
