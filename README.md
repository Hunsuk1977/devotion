...
`/admin` 에는 탭이 두 개 있습니다.

- **글 쓰기** — 묵상집·날짜·언어를 고르고 한 편을 씁니다.
- **묵상집 관리** — 묵상집을 만들고, 이름과 소개를 고치고, 순서를 바꿉니다.

직접 쓰는 묵상은 **먼저 묵상집을 만들고, 그 묵상집에 날짜별로 한 편씩** 쓰는 흐름입니다. 새 묵상집을 저장하면 `collections.json` 에 커밋되고, 첫 글을 올리는 순간 독자 화면에 탭이 생깁니다 (글이 없는 묵상집은 독자에게 보이지 않습니다). 이메일에서 오는 묵상집은 지금처럼 워크플로가 알아서 채웁니다.
